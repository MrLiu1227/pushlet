...
function initDict(dicts){
	...
	if(dfDictName){
		...
		initDictItem(dfDictName,oNode && getEditortCheckbox(oNode,dfPluginType + '_' + uuid));
	}
}
    ...
    $.ajax({
        ...
        success: function(data){
        	if(data.result == 0){
        		...
        		//setCheckboxValue(dfPluginType + '_' + uuid, value);
        		setCheckboxChecked(dfPluginType + '_' + uuid,selected);
        	}else{
        		alert(data.msg);
        	}
        }
...
function buildRadios(dicts){
	...
	for ( var i = 0; i < dicts.length; i++ ){
		var dict = dicts[i];
		options += '<input name="' + dfPluginType + '_' + uuid + '" value="' + dict.code + '" type="checkbox" />' + dict.name + '&nbsp;';
	}
	html += options;
	html += '</span>';
	return html;
...
dialog.onok = function (){
	...
	setCheckboxChecked(dfPluginType + '_' + uuid);
    if( !oNode ) {
        try {
