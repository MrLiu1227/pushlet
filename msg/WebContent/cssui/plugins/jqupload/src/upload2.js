...
(function($) {
	...
	$.fn.jqupload = function(o) {
		var defaults = {
			...
			dataType : 'json',
			method : 'POST',
			uploadType : 'async', // sync:同步 , async: 异步（失败概率大）
			/**
			 * picMethod 扩展
			 */
			imgPreview : false,
		...
		$.post(o.loadUrl, o.extraData, function(data) {
			switch (data.result) {
				case 0:
					...
					var dataArray = data.msg.data.list[0].attachments;
		...
		return this.each(function() {
			...
			if (batchFlag) {
				...
				_extraData.tableUuid = $this.attr('data-id');
			}
				...
				$.post(o.loadUrl, o.extraData, function(data) {
					switch (data.result) {
						case 0:
							...
							/**
							 * {type: 'crop',width : 180,height: 200 ,previewWidth : [
							 * 180, 120, 60 ],cropWidth : [ 180, 120, 60 ]}";
							 */
							// console.log(o.extraPara);
							if (!isnull(o.extraPara)) {
								if (o.extraPara.type == 'pic') {
									...
									if (!o.readonly) {
										// 剪裁时IE6, 7, 8, 9先上传
										if (!(dataArray instanceof Array || !isnull(dataArray)) && (isIE6 || isIE7 || isIE8 || isIE9))
											initEvent();
										else {
											initButton();
											initCropEvent(false);
